    ...
    for grp in ['climp', 'ctrl', 'rtn']:
        ...
        for series in series_names:
            ...
            for file in ser_files:
                ...
                # exit()
                area_data.extend(area_val['area'] for area_val in features)
                axis_major_length_data.extend(ax_major_val['axis_major_length'] for ax_major_val in features)

                axis_minor_length_data.extend(ax_min_val['axis_major_length'] for ax_min_val in features)

                area_convex_data.extend(area_convex_val['area_convex'] for area_convex_val in features)
...
def fwhm_analysis(grp):
    ...
    @return:
    """
    with open(dirs_path + f'{grp}_intensity.pkl', 'rb') as fl:
...
def rtn_intensity_profiles_plos(group):
    ...
    for i in range(1, len(ser221)):
        try:
            ...
            plt.title('Intensity profile, ' + group + f'_Series037_1_id_{i}')
            plt.plot(tubule)
            ...
            # plt.show()
            plt.savefig('/localhome/asa420/MIAL/data/CROP-n/Plos_data_crops/intensity_profiles/' + group + f'_Series037_1_id_{i}')
...
def fwhm_analysis_plos(grp):
    with open(plos_data_path + f'{grp}_plos_skel_intensity.pkl', 'rb') as fl:
        intensity_data = pickle.load(fl)
...
def fwhm_analysis_fr(grp):
    with open(dirs_path + f'{grp}_match_dil_intensity.pkl', 'rb') as fl:
        intensity_data = pickle.load(fl)
