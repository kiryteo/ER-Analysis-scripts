...
def mean_proj_input():
    ...
    for ser_num in range(1, 32):
        ...
        for i in range(100):
            img = imageio.imread(f'/localhome/asa420/MIAL/data/confocal_movies/RTN/files/R{ser_num}_decon_t0{i:02d}_ch00.tif')

            img = (img - img.min())/(img.max() - img.min())
            er_mean += img
        cv2.imwrite(f'rtn{ser_num}_er_mean.png', (er_mean/100)*255)
...
def preprocess_samples():
    ...
    for i in range(27, 32):
        img = imageio.imread(f'/localhome/asa420/MIAL/data/confocal_movies/Control/new_op_jul/er_mean/control{i}_er_mean.png')
        ...
        cv2.imwrite(f'/localhome/asa420/MIAL/data/confocal_movies/Control/new_op_jul/er_mean_proc/control{i}_er_mean_proc.png', loc)


def simp_fourier():
    for i in range(100):
        img = imageio.imread(f'/localhome/asa420/MIAL/data/confocal_movies/ATL/files/A1_decon_t0{i:02d}_ch00.tif')
...
def junction_location_plotter():
    ...
    newps = [ps[i] for i, val in enumerate(degree_list) if val[1] > 2]
    # print(newps)
    ...
    for i in range(100):
        img = imageio.imread(f'/localhome/asa420/MIAL/data/confocal_movies/ATL/new_op_jul/skel/A1/A1_decon_t0{i:02d}_ch00_skel.png')
...
def count_connected_components():
    ...
    for num in range(1, 31):
        for i in range(100):
            path = f'/localhome/asa420/MIAL/data/confocal_movies/Control/new_op_jul/skel/Ct{num}/Ct{num}_decon_t0{i:02d}_ch00_skel.png'
    ...
    for num in range(1, 27):
        for i in range(100):
            path = f'/localhome/asa420/MIAL/data/confocal_movies/ATL/new_op_jul/skel/A{num}/A{num}_decon_t0{i:02d}_ch00_skel.png'
    ...
    for num in range(1, 31):
        for i in range(100):
            path = f'/localhome/asa420/MIAL/data/confocal_movies/Climp/new_op_jul/skel/C{num}/C{num}_decon_t0{i:02d}_ch00_skel.png'
    ...
    for num in range(1, 30):
        for i in range(100):
            path = f'/localhome/asa420/MIAL/data/confocal_movies/RTN/new_op_jul/skel/R{num}/R{num}_decon_t0{i:02d}_ch00_skel.png'
...
def junction_analysis(group, channel):
    ...
    if group == 'ATL':
        for series_num in range(1, 27):
            newps = junction_flow(f'{prefix}ATL/new_op_jul/ATL_mean_proj/A{series_num}_mean.png')

            for i in range(100):
                path = f'{prefix}ATL/files/A{series_num}_decon_t0{i:02d}_ch0{channel}.tif'
        ...
        for series_num in range(1, 31):
            newps = junction_flow(f'{prefix}Climp/new_op_jul/Climp_mean_proj/C{series_num}_mean.png')

            for i in range(100):
                path = f'{prefix}Climp/files/C{series_num}_decon_t0{i:02d}_ch0{channel}.tif'
        ...
        for series_num in range(1, 31):
            newps = junction_flow(f'{prefix}Control/new_op_jul/Ctrl_mean_proj/Ct{series_num}_mean.png')

            for i in range(100):
                path = f'{prefix}Control/files/img_{series_num}_decon_t0{i:02d}.tif'
        ...
        for series_num in range(1, 30):
            newps = junction_flow(f'{prefix}RTN/new_op_jul/RTN_mean_proj/R{series_num}_mean.png')

            for i in range(100):
                path = f'{prefix}RTN/files/R{series_num}_decon_t0{i:02d}_ch0{channel}.tif'
...
def dil_junctions():
    ...
    for series_num in range(1, 2):
        newps, brpts = junction_flow(f'{prefix}ATL/new_op_jul/ATL_mean_proj/A{series_num}_mean.png')
...
n = [fft_abs[i] for i in range(100)]
plt.plot(n)
...
def tub_analysis(mean_img):
    ...
    return skimage.measure.label(tubules)
...
def get_tubule_XY(lab_tubules):
    ...
    return {num: list(zip(pts[0], pts[1])) for num, pts in lt.items()}
...
def get_ndt(series_num, coords, group):
    ...
    for tubule_num, coord_list in coords.items():
        # print(coord_list)
        ndt[tubule_num] = []
        if group in ['ATL', 'Climp', 'RTN']:
            path = f'{pref}{group}/files/{group[0]}{series_num}_decon_t0'
        else:
            path = f'{pref}{group}/files/img_{series_num}_decon_t0'

        for i in range(100):
            if group == 'Control':
                fpath = path + f'{i:02d}.tif'
            else:
                fpath = path + f'{i:02d}_ch00.tif'
...
def get_ndt_mean(ndt):
    return [list(np.mean(v, axis=0)) for k, v in ndt.items()]
...
def variance_analysis():
    ATL_vals = []
    for series_num in range(1, 27):
        mean_img = f'/localhome/asa420/MIAL/data/confocal_movies/ATL/new_op_jul/ATL_mean_proj/A{series_num}_mean.png'
    ...
    Climp_vals = []
    for series_num in range(1, 31):
        mean_img = f'/localhome/asa420/MIAL/data/confocal_movies/Climp/new_op_jul/Climp_mean_proj/C{series_num}_mean.png'
    ...
    Ctrl_vals = []
    for series_num in range(1, 31):
        mean_img = f'/localhome/asa420/MIAL/data/confocal_movies/Control/new_op_jul/Ctrl_mean_proj/Ct{series_num}_mean.png'
    ...
    RTN_vals = []
    for series_num in range(1, 30):
        mean_img = f'/localhome/asa420/MIAL/data/confocal_movies/RTN/new_op_jul/RTN_mean_proj/R{series_num}_mean.png'
...
var_list = [np.var(mean_list[i]) for i, val in enumerate(mean_list)]
# plt.hist(var_list)
import seaborn as sns
