...
def preprocess_samples(group):
    ...
    new_pref = f'/localhome/asa420/MIAL/data/confocal_movies/{group}/new_op_jul/preproc_mcherry/'


    for i in range(1, 27):
        os.makedirs(f'{new_pref}A{i}')
        for j in range(100):
            img = imageio.imread(f'{path_pref}A{i}_decon_t0{j:02d}_ch01.tif')
            ...
            cv2.imwrite(f'{new_pref}A{i}/A{i}_decon_t0{j:02d}_ch01_proc_op5.png', loc)
...
def get_skel():
    ...
    for i in range(1, 27):
        os.makedirs(f'{pref}skel_mcherry/A{i}')
        for j in range(100):
            img = imageio.imread(f'{pref}preproc_mcherry/A{i}/A{i}_decon_t0{j:02d}_ch01_proc_enhance.png')

            sk = pcv.morphology.skeletonize(img)
            cv2.imwrite(f'{pref}skel_mcherry/A{i}/A{i}_decon_t0{j:02d}_ch01_skel.png', sk)
