...
def runner():
    for each in dire:
        ...
        # series = int(each[1:])
        if series < 10:
            dname = f'{erpath}Series00{str(series)}_decon_converted/erenh/'
        else:
            dname = f'{erpath}Series0{str(series)}_decon_converted/erenh/'
        files = glob.glob(f'{each}/*')
        dt = {}
        # skel_list = []
        ...
        dt[dirname] = [mean_er_skel, mean_junc]
        # dt[dirname].append(maxjunc)
        #
        dtname = f'{dirname}.pickle'
