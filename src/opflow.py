...
def get_curls(group):
    ...
    files = glob.glob(f'/localhome/asa420/MIAL/data/confocal_movies/{group}/new_op_jul/opflow_files/*')
...
def flow_analysis_er(group, prop):
    ...
    pref = f'/localhome/asa420/MIAL/data/confocal_movies/{group}/new_op_jul/intensity_based_flow_{prop}/'
    ...
    for series in range(1, grp_num[group]):
        for frame in range(99):
            prop_data = scipy.io.loadmat(pref + f'{grp_pref[group]}{series}_{frame}_opfl_{prop}.mat')['c']

            img = imageio.imread(f'/localhome/asa420/MIAL/data/confocal_movies/{group}/new_op_jul/junctions/{grp_pref[group]}{series}/{grp_pref[group]}{series}_decon_t0{frame:02d}_ch00_junc.png')
...
def flow_magnitude_analysis_er(group):
    ...
    pref = f'/localhome/asa420/MIAL/data/confocal_movies/{group}/new_op_jul/intensity_based_flow/'
    ...
    for series in range(1, grp_num[group]):
        for frame in range(99):
            curl_data = scipy.io.loadmat(pref + f'{grp_pref[group]}{series}_{frame}_opfl.mat')['m']
...
def curl_analysis(group):
    ...
    """
    pref = f'/localhome/asa420/MIAL/data/confocal_movies/{group}/new_op_jul/opflow_files/'
    ...
    for series in range(1, grp_num[group]):
        for frame in range(99):
            curl_data = scipy.io.loadmat(pref + f'{grp_pref[group]}{series}_{frame}_flow.mat')['c']
...
def get_div(group):
    ...
    files = glob.glob(f'/localhome/asa420/MIAL/data/confocal_movies/{group}/new_op_jul/opflow_div/*')
