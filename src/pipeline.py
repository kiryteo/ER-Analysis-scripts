...
prefix = f'{home}/MIAL/KV_paired_ER_tubules/STED'
...
def pipeline_creator(group, file_num):
	...
	sample = f'{prefix}/{group}/samples/{grp_pref}{str(file_num)}_decon_ch02.tif'
	grp_sample = mpimg.imread(sample)
	plt.imshow(grp_sample)
	plt.show()

	enhanced_sample = f'{prefix}/{group}/enh/{grp_pref}{str(file_num)}_decon_ch02_std_enhance.png'

	grp_enh = mpimg.imread(enhanced_sample)
	plt.imshow(grp_enh)
	plt.show()

	tubules = f'{prefix}/{group}/tub_dil/{grp_pref}{str(file_num)}_decon_ch02_std_enhance_tubules.png'

	grp_tub = mpimg.imread(tubules)
	plt.imshow(grp_tub)
	plt.show()

	selected_tubules = f'{prefix}/{group}/selection_dil/{grp_pref}{str(file_num)}_decon_ch02_std_enhance_tubules_labeled_selected.png'
