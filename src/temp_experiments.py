...
def run_matching(grp, ser_num):
    ...
    ref_img_name = f'{dir_pref}/Decon/Series{ser_num:03d}_decon_converted/std/Series{ser_num:03d}_decon_converted_t00_ch00_std.png'

    ref_img = imageio.imread(ref_img_name)

    img_path = f'{dir_pref}/Decon/Series{ser_num:03d}_decon_converted/std/'

    matched_dir = f'{dir_pref}/Decon/Series{ser_num:03d}_decon_converted/matching/'
    os.makedirs(matched_dir)

    shutil.copy(ref_img_name, matched_dir)

    for i in range(1, 100):
        imgname = f'{img_path}Series{ser_num:03d}_decon_converted_t{i:02d}_ch00_std.png'
...
def interfr_corr():
    ...
    l = []
    for i in range(2,3):
        im1 = io.imread(f'{imdir}Series002_decon_converted_t{i:02d}_ch00_std.png')
        for j in range(i+1, 100):
            im2 = io.imread(f'{imdir}Series002_decon_converted_t{j:02d}_ch00_std.png')
...
def get_hausdorff_dist(grp, num, frame):
    ...
    if grp == 'climp':
        gt = imageio.imread(f'{pref}Climp/Climp_Series{num}/Series0{num}_t{frame:02d}.png')

        pred = imageio.imread(f'{pref}Climp/Climp_Series{num}/Series0{num}_decon_converted_t{frame:02d}_ch00_std_enhance_skel.png')

    elif grp == 'ctrl':
        gt = imageio.imread(f'{pref}Control/Ctrl_Series{num}/Series0{num}_t{frame:02d}.png')

        pred = imageio.imread(f'{pref}Control/Ctrl_Series{num}/Series0{num}_decon_converted_t{frame:02d}_ch00_std_enhance_skel.png')

    else:
        gt = imageio.imread(f'{pref}RTN/RTN_Series{num}/Series00{num}_t{frame:02d}.png')

        pred = imageio.imread(f'{pref}RTN/RTN_Series{num}/Series00{num}_decon_converted_t{frame:02d}_ch00_std_enhance_skel.png')
