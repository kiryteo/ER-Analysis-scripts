...
def ref_junc_data_per_group(group, num_series, channel, region):
    ...
    for num in range(1, num_series+1):
        ...
        # store per movie list of ref junc list with 100 values
        data = []
        for CC_ref in region_data:
            ...
            for i in range(100):
                input_data = imageio.imread(f'{confocal_data_path}/{group}/new_op_jul/{dirname}/{group_pref[group]}{num}_decon_t0{i:02d}_ch{ch:02d}_std.png')
                junc_signal_vals.append(input_data[CC_ref[0], CC_ref[1]])
