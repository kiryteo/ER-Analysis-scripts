...
# Fixed cell data
conf_data_8 = pd.read_csv('/localhome/asa420/MIAL/Series008_decon_ch00.csv')
sted_data_8 = pd.read_csv('/localhome/asa420/MIAL/Series008_decon_ch01.csv')
...
sns.stripplot(x=conf_data_8['Perimeter'], color='red', label='Confocal', ax=axes[0,0], size=1)
sns.stripplot(x=sted_data_8['Perimeter'], color='blue', label='STED', ax=axes[0,0], size=1)
axes[0,0].set_title('Series008_decon', size=10)
axes[0,0].set_xlabel('Perimeter', fontsize=10)
...
plt.suptitle('Ctrl (group), STED and Confocal (mod), Perimeter (feature) across multiple samples')
plt.show()
